...
- **改动**：
  - 后端 `larkImReplyService.sendImReplyFromCard` **整段硬禁**（一律抛"公司策略不允许 AI 代发飞书消息"，
    绝不调 lark-cli；删除发送死代码）。`previewImReply`（解析"回复给谁"+草稿）保留——这是 IM 这块的留存价值。
  - 前端 SignalCard：「代我回复飞书」→「🤖 AI 起草回复」；「确认发送」→「复制草稿」（navigator.clipboard），
    成功提示改"已复制草稿，去飞书粘贴发送"。AI 起草价值保留，发送动作去掉。
- **验证**：server tsc 干净（排除并行会话 readTools）、web tsc ✓；MVP34 测试改为"send 一律硬禁 + preview 仍可用"，
  4 测过；**preview 实测前端按钮已变「🤖 AI 起草回复」、旧文案消失**；全量见下。
- **方向**：自主完成的发力点收敛到**允许的执行动作**——建飞书文档/任务（已授权）、办结（MVP39）、自主排查；
  对外 IM 一律只起草、用户手动发。

### 2026-06-15 第 36 轮：排查回写不再污染卡片（满意度修复，证据优先）

- **体检发现真问题**：试运行 12/15 是 unknown，但 investigationWriteback 对**每次**排查（含 unknown）都
  ① 把结论塞进 matter 摘要首行、② 刷新 lastEvidenceAt。实测后果：6 个事项摘要被污染——首行变「[AI 排查]
  未查到证据」（噪音）、**内部失败「排查器未给出有效动作」泄漏给用户**、多次排查 `[AI 排查]…｜[AI 排查]…`
  无限堆叠、陈旧度全失真（55 个 open 全 <7 天，部分是被排查刷新的假象）。直接伤满意度。
- **修复（全在 investigationWriteback.ts，我的文件）**：只有**有意义结论**（resolved/progressed/blocked）
  才动用户可见字段；unknown 一律不碰摘要/nextAction/证据时钟，只留审计。更新摘要时**剥掉旧的「[AI 排查]…｜」
  前缀**（替换而非堆叠）。summaryUpdated 对 unknown 返回 false。
- **数据修复**：清掉 5 个被污染摘要的噪音前缀（内部失败「排查器未给出有效动作」泄漏清零；真发现保留）。
- **验证**：tsc 干净；回写测试 +2（unknown 不污染摘要/不刷新时钟、多次不堆叠），共 10 测；全量 **604/604**。
  只提交自己的文件（避开并行会话 readTools/config）。
- **逼近目标**：卡片更干净可信 → 满意度↑；陈旧度恢复真实 → 后续可据此做陈旧三选/优先级。
