...
export async function sendImReplyFromCard(
  ...
  if (!input.confirm) {
    throw new Error('confirm is required before sending a Lark message');
  }
  const source = resolveSource(input.cardId);
  const target = resolveImReplyTarget(source);

  const text = (input.text ?? source.draftReply ?? '').trim();
  if (!text) throw new Error('回复内容为空，拒绝发送');
  if (text.length > MAX_TEXT_LEN) {
    throw new Error(`回复内容过长（${text.length} > ${MAX_TEXT_LEN}）`);
  }

  const idempotencyKey = createHash('sha256')
    .update(`im-reply:${source.sourceRefId}:${text}`)
    .digest('hex')
    .slice(0, 32);

  const baseArgs = target.messageId
    ? ['im', '+messages-reply', '--message-id', target.messageId, '--text', text, '--idempotency-key', idempotencyKey, '--format', 'json']
    : ['im', '+messages-send', '--chat-id', target.chatId, '--text', text, '--idempotency-key', idempotencyKey, '--format', 'json'];
  const args = input.dryRun ? [...baseArgs, '--dry-run'] : baseArgs;
  ...
  try {
    ...
    if (code !== 0) {
      throw new Error(`lark-cli exit ${code}: ${(stderr || stdout).slice(-300)}`);
    }
    try {
      const parsed = JSON.parse(stdout) as Record<string, unknown>;
      sentMessageId = findStringByKey(parsed, ['message_id', 'messageId', 'om_message_id']);
    } catch {
      // 非 JSON 输出不阻塞：发送可能已成功（idempotency-key 兜底重试安全）
    }
  } catch (err) {
    const rawMsg = err instanceof Error ? err.message : String(err);
    // 缺「以用户身份发消息」scope 是可自助解锁的——给出清晰可操作的提示，而非 cryptic 报错。
    const friendly = /im:message\.send_as_user|missing_scopes/.test(rawMsg)
      ? '尚未授权「以你的身份发送飞书消息」。请在终端运行：lark-cli auth login --scope "im:message.send_as_user"，按提示在浏览器完成验证后重试。（其余功能不受影响）'
      : rawMsg;
    writeAudit({
      action: 'im_reply_failed',
      reason: `代发飞书回复${input.dryRun ? '(dry-run)' : ''}失败：${target.chatName ?? target.chatId}`,
  ...
  // dry-run：真实 lark-cli 已受理命令（鉴权+目标校验通过），但未实际发送 —— 不写回/不标记/不审计为已发。
  if (input.dryRun) {
    return { ok: true, target, sentMessageId, resultUnitId: '', dryRun: true };
  }

  // 回写：silent action_result（记录"我已回复"），挂到 Matter 作证据。
  const chatEntities: ContextEntityRef[] = [
    { type: 'chat', name: `lark_chat:${target.chatId}`, role: 'container', confidence: 1 },
  ];
  const resultUnit = upsertContextUnit({
    kind: 'action_result',
    title: `已回复${target.chatName ? `「${target.chatName}」` : ''}`,
    content: [
      `已通过飞书发送回复：`,
      text,
      ``,
      `会话：${target.chatName ?? target.chatId}`,
      target.replyToText ? `回应原话：${target.replyToText}` : '',
    ]
      .filter(Boolean)
      .join('\n'),
    ...
    confidence: 1,
    mergeHint: `im-reply:${source.sourceRefId}:${idempotencyKey}`,
    silent: true,
  }).unit;

  const now = new Date().toISOString();
  const fullMatterId = source.matterId ? matchMatterId(source.matterId) : null;
  if (fullMatterId && getMatterById(fullMatterId)) {
    attachMatterContextLink({
      ...
      effect: 'no_change',
      confidence: 1,
      reason: 'AI 代发飞书回复（用户确认）',
      now,
    });
  }

  writeAudit({
    action: 'im_reply_sent',
    reason: `已代发飞书回复：${target.chatName ?? target.chatId}`,
    cardId: source.sourceRefId,
    payload: {
      chatId: target.chatId,
      messageId: target.messageId,
  ...
  // 卡片置 acted（attention）/ acknowledged（老 cards 表）。
  let card: SignalCard | undefined;
  if (source.sourceKind === 'attention') {
    const attn = getAttentionItem(source.sourceRefId);
    if (attn) {
      recordAttentionInteraction(attn, 'send_reply', now);
      const updated = updateAttentionItemStatus(attn.id, 'acted', now);
      if (updated) {
        card = projectAttentionItemToCard(updated);
  ...
  return { ok: true, target, sentMessageId, resultUnitId: resultUnit.id, card };
}
