...
const BACKLOG_SWEEP_SYSTEM = `你是「积压大扫除甄别官」。系统给你一批已停滞多天的工作事项（编号+标题+摘要+原定下一步）。你的唯一任务：仅凭给出的信息逐件判断它现在的状态，输出 JSON 数组。
...
const AGENTS: readonly AgentDef[] = [
  ...
  {
    // MVP78 积压大扫除甄别官：只判断不执行（清理由用户在飞书一句话确认）。全 deny：纯文本进、JSON 出。
    name: 'aiisn-backlog-sweep',
    description: 'AI is ON MVP78 积压大扫除甄别官（陈旧事项 → likely_done/event_passed/obsolete/still_pending）',
    permission: { bash: 'deny', edit: 'deny', write: 'deny', webfetch: 'deny', read: 'deny' },
    prompt: BACKLOG_SWEEP_SYSTEM,
  },
];
