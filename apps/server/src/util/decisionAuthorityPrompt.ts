/**
 * MVP15B M3 — 推断某人对某项目的决策权重（high / mid / low）。
 *
 * 输入：一组 person-project 边的证据：
 *   - personName + 可选 orgRole / biz / fn
 *   - projectCanonicalName
 *   - SQL 已推断的 role (owner/driver/reviewer/contributor/stakeholder/observer)
 *   - 最近 5 条 evidence unit 的 title + meaning
 *
 * 输出：每条边的 decisionAuthority + 一句话 why。
 */

export const DECISION_AUTHORITY_SYSTEM_PROMPT = `你是项目决策权判定器。
