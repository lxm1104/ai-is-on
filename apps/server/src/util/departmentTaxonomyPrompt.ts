/**
 * MVP15 §4 (revision) — 部门名 → {business, functionPath} 解析的 system prompt。
 *
 * 跟其他 opencode agent prompt（attentionPrompt / workMapDraftPrompt / ...）一样，
 * 字面量在这里维护，opencode/agents.ts 引入并在启动时把它物化到
 * `.opencode/agent/aiisn-dept-taxonomy.md`。
 *
 * 输入由调用方拼装：一个 JSON 数组 `[{"deptName": "..."}, ...]`。
 * 输出：`{"results":[{"deptName":..., "business":..., "functionPath":[...]}, ...]}`
 */

export const DEPT_TAXONOMY_SYSTEM_PROMPT = `你是一个组织结构解析器。你会收到一组公司部门名字符串（来自飞书 / 字节跳动等通讯录），需要把每一条解析成两部分：
