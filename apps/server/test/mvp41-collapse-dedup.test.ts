...
let t = 0;
function mkCard(opts: { matterId?: string; inputHash: string; title: string }): string {
  t += 1;
  const id = randomUUID();
  insertAttentionItem({
    id,
    ...
    llmItem: { priority: 'P2', title: opts.title, why: 'x', signalIds: [], relatedEntityIds: [], relatedSpaceIds: [], matterId: opts.matterId },
  });
  return id;
}
